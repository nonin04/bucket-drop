...
- **.drift ファイルへの集約 (sqlc スタイル)**:
  - **テーブル定義とクエリ**: テーブル定義（DDL）および動的な組み立てを伴わない DB 操作（CRUDクエリ）は、すべて `.drift` ファイル内に記述する（Dart でのメソッドチェーン ORM は禁止）。
  - **データクラスの命名**: ドメインモデルとの衝突を防ぐため、`AS` 句を用いて `CREATE TABLE <テーブル名> (...) AS <モデル名>Table;` の形式で定義し、自動生成されるテーブル行クラスを `〇〇Table` に統一する。
  - **主キー (PK)**: 整合性維持および削除後の ID 再利用防止のため、原則として `id INT NOT NULL PRIMARY KEY AUTOINCREMENT` を付与する。
